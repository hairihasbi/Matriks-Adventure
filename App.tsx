...
export default function App() {
  ...
  const [currentDialogue, setCurrentDialogue] = useState(
    STORY_DATA.dialogues.welcome,
  );
  const [showGuide, setShowGuide] = useState(false);
  const [viewMode, setViewMode] = useState<"map" | "content" | "quiz" | "lab">(
    "map",
  );
  ...
  const [storageConnected, setStorageConnected] = useState<boolean | null>(
    null,
  );
  ...
  useEffect(() => {
    if (currentStage && currentStage.quizzes) {
      const filteredQuizzes = currentStage.quizzes.filter(
        (q) => q.difficulty === gameState.difficulty,
      );
      const shuffled = shuffle(
        filteredQuizzes.length > 0 ? filteredQuizzes : currentStage.quizzes,
      ).map((q) => ({
        ...q,
  ...
  const currentSubTopic =
    currentStage.subTopics[gameState.currentSubTopicIndex];
  ...
  const handleExperimentCount = () => {
    setGameState((prev) => {
      ...
      if (newCount === 10 && !currentAchievements.includes("scientist")) {
        ...
        return {
          ...prev,
          labExperiments: newCount,
          achievements: [...currentAchievements, "scientist"],
        };
      }
  ...
  const handleNextQuiz = (isCorrect: boolean) => {
    ...
    if (isCorrect) {
      ...
      if (newStreakCount === 5 && !currentAchievements.includes("flash")) {
        setGameState((prev) => ({
          ...prev,
          achievements: [...(prev.achievements || []), "flash"],
        }));
        celebrateBigWin();
      }
      ...
      setGameState((prev) => {
        ...
        const availableRewards = REWARDS.filter(
          (r) => r.xp <= newScore && !prev.unlockedRewards.includes(r.id),
        );
  ...
  const handleNextStage = () => {
    if (gameState.currentStageIndex < MATRIX_ADVENTURE_DATA.length - 1) {
      ...
      if (gameState.currentStageIndex + 1 === 4) {
        // Stage 5 (index 4)
        setCurrentDialogue(STORY_DATA.dialogues.advancedWarning);
      } else if (gameState.currentStageIndex + 1 === 5) {
        // Stage 6 (index 5)
      ...
      const currentAchievements = gameState.achievements || [];
      if (
        gameState.currentStageIndex === 3 &&
        gameState.difficulty === Difficulty.HARD &&
        !currentAchievements.includes("grandmaster")
      ) {
        setGameState((prev) => ({
          ...prev,
          achievements: [...(prev.achievements || []), "grandmaster"],
        }));
        celebrateLevelUp();
      }
  ...
  const masteryPercentage = Math.round(
    ((completedSubTopics.length +
      (gameState.isQuizActive
        ? currentQuizIndex
        : stageFinished
          ? shuffledQuizzes.length
          : 0)) /
      (currentStage.subTopics.length + shuffledQuizzes.length)) *
      100,
  );
        ...
        >
          ...
          <div className="flex items-center justify-center gap-2 mb-4">
            {dbConnected !== null && (
              ...
              >
                <Database
                  className={dbConnected ? "w-3 h-3 animate-pulse" : "w-3 h-3"}
                />
            ...
            {storageConnected !== null && (
              ...
              >
                <Layers
                  className={
                    storageConnected ? "w-3 h-3 animate-pulse" : "w-3 h-3"
                  }
                />
          ...
          >
            ...
            <div className="bg-white/5 p-5 md:p-6 rounded-3xl border border-white/10 mb-8 backdrop-blur-md relative overflow-hidden group/quote mx-4 md:mx-0">
              ...
              <p className="text-xs md:text-base text-white/70 italic font-medium">
                "{STORY_DATA.intro}"
              </p>
            </div>
            ...
            <div className="flex flex-col gap-8 px-6 md:px-0">
              ...
              <div className="flex flex-col md:flex-row items-center justify-center gap-4">
                <button
                  ...
                  className="w-full md:w-auto px-6 py-3 bg-white text-indigo-700 rounded-xl font-black text-base md:text-lg hover:scale-105 hover:bg-yellow-300 hover:text-indigo-900 transition-all shadow-lg active:scale-95 flex items-center justify-center gap-2 touch-manipulation"
                >
                  Ayo Mulai!
                  <ArrowRight className="w-4 h-4 md:w-5 md:h-5 stroke-[4]" />
                </button>

                <button
                  onClick={() => setShowObjectives(true)}
                  className="w-full md:w-auto px-4 py-2.5 bg-indigo-600/30 text-white border border-white/20 rounded-xl font-black text-xs md:text-sm hover:bg-white/10 transition-all flex items-center justify-center gap-1.5 active:scale-95 backdrop-blur-sm"
                >
                ...
                <button
                  ...
                  className="w-full md:w-auto px-4 py-2.5 bg-purple-600/30 text-white border border-white/20 rounded-xl font-black text-xs md:text-sm hover:bg-white/10 transition-all flex items-center justify-center gap-1.5 active:scale-95 backdrop-blur-sm"
                >
                ...
                <button
                  ...
                  className="w-full md:w-auto px-4 py-2.5 bg-blue-600/30 text-white border border-white/20 rounded-xl font-black text-xs md:text-sm hover:bg-white/10 transition-all flex items-center justify-center gap-1.5 active:scale-95 backdrop-blur-sm"
                >
            ...
            <div className="absolute -bottom-3 -right-3 opacity-20 pointer-events-none scale-150 rotate-12 hidden md:block">
              <div className="grid grid-cols-2 gap-2">
                {[...Array(4)].map((_, i) => (
                  <div key={i} className="w-8 h-8 bg-white rounded-lg"></div>
                ))}
              </div>
            </div>
          </div>

          <div className="flex items-center justify-center gap-2 text-white/40 text-[10px] font-black uppercase tracking-[0.4em] mb-12">
    ...
    return (
      <div className="min-h-screen relative flex flex-col items-center justify-center p-4 overflow-hidden">
        ...
        >
          ...
          <h2 className="text-3xl md:text-5xl font-black text-white mb-2 leading-none uppercase italic tracking-tighter">
            LEVEL SELESAI!
          </h2>
          <p className="text-white/70 mb-6 md:mb-8 text-base md:text-lg font-medium px-4">
          ...
          <div className="bg-white/10 backdrop-blur-md p-5 md:p-6 rounded-2xl md:rounded-3xl mb-8 md:mb-10 border border-white/20 relative group">
            ...
            <span className="text-[10px] text-white/50 uppercase font-black tracking-[0.3em] block mb-1">
              Skor Saat Ini
            </span>
            <span className="text-4xl md:text-6xl font-black text-white drop-shadow-sm">
              {gameState.score} XP
            </span>
          </div>
    ...
    return (
      <div className="min-h-screen relative flex flex-col items-center justify-center p-4">
        ...
        >
          ...
          <h2 className="text-4xl font-black text-white mb-3 leading-none">
            MASTER MATRIKS!
          </h2>
          <p className="text-white/70 mb-8 text-lg font-medium">
            Kamu telah menuntun penjelajah melewati gerbang konsep pertama.
          </p>

          <div className="bg-white/10 backdrop-blur-md p-8 rounded-3xl mb-10 border border-white/20">
            <span className="text-xs text-white/50 uppercase font-black tracking-[0.3em] block mb-2">
              Total Pengalaman
            </span>
          ...
          {gameState.unlockedRewards.length > 0 && (
            <div className="mb-10 text-left">
              <h4 className="text-white font-black text-xs uppercase tracking-widest mb-4 opacity-60">
                Pencapaian Unlocked
              </h4>
              <div className="flex flex-wrap gap-3">
                {REWARDS.filter((r) =>
                  gameState.unlockedRewards.includes(r.id),
                ).map((reward) => (
                  <div
                    key={reward.id}
                    className="bg-white/20 px-4 py-2 rounded-xl flex items-center gap-2 border border-white/20"
                  >
          ...
          <div className="flex flex-col gap-4">
            ...
            <button
              ...
              onClick={() =>
                alert(
                  "Dunia berikutnya sedang dibangun oleh para penyihir matematika!",
                )
              }
            >
              Ke Pulau Berikutnya
    ...
    <div className="min-h-screen relative overflow-x-hidden flex flex-col p-4 md:p-8 gap-6">
      ...
      <nav className="relative z-10 flex flex-col md:flex-row items-center justify-between bg-white/20 backdrop-blur-xl border border-white/30 rounded-[2rem] px-8 py-5 shadow-2xl gap-4">
        <div className="flex items-center gap-4">
          ...
          >
            <FlaskConical
              className={`w-6 h-6 group-hover:scale-110 transition-transform ${viewMode === "lab" ? "text-white" : "text-white/70"}`}
            />
          </button>
          ...
          <div>
            <h1 className="text-white font-black text-2xl tracking-tight leading-none uppercase">
              Matriks Adventure
            </h1>
            <p className="text-white/60 text-[10px] font-black tracking-[0.2em] uppercase mt-1">
              Petualangan Matriks
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-8 w-full md:w-auto">
          <div className="flex-1 md:flex-none flex flex-col items-end gap-1">
            <div className="flex gap-2 items-center">
              ...
              <span className="text-white font-black text-[10px] uppercase tracking-wider">
                Level {gameState.currentStageIndex + 1}
              </span>
            </div>
          </div>
          <div className="bg-white/10 px-6 py-2 rounded-2xl border border-white/20 backdrop-blur-md flex items-center gap-4">
            ...
            <div className="flex flex-col items-center">
              <span className="text-[8px] text-white/50 font-black uppercase tracking-widest">
                Kesulitan
              </span>
      ...
      <main className="relative z-10 flex-1 grid grid-cols-1 lg:grid-cols-12 gap-6 min-h-0">
        ...
        <aside className="col-span-1 lg:col-span-3 flex flex-col gap-4 md:gap-6 order-2 lg:order-1">
          <div className="bg-white/10 backdrop-blur-xl border border-white/20 rounded-[2.5rem] p-6 flex flex-col gap-3">
            <h3 className="text-white/50 font-black text-[10px] tracking-[0.2em] uppercase mb-3 ml-2">
              Status Quest
            </h3>
            ...
            <div className="p-4 bg-indigo-500/30 rounded-2xl border border-white/10">
              <p className="text-white/70 text-[10px] font-black uppercase mb-1">
                Misi Aktif
              </p>
              <p className="text-white font-bold text-sm leading-tight">
                Pulihkan Gerbang Matriks Pulau
              </p>
            </div>

            {/* Stage Selector List */}
            <h3 className="text-white/50 font-black text-[10px] tracking-[0.2em] uppercase mt-6 mb-3 ml-2">
              Progress Wilayah
            </h3>
            <div className="space-y-2 overflow-y-auto max-h-[300px] pr-2 scrollbar-hide">
              {MATRIX_ADVENTURE_DATA.map((stage, idx) => {
                ...
                const isCompleted = gameState.completedStages.includes(
                  stage.id,
                );
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div
                        className={`w-8 h-8 rounded-full flex items-center justify-center font-black text-xs ${isCompleted ? "bg-emerald-400 text-indigo-900" : isActive ? "bg-white text-indigo-600" : "bg-white/10 text-white/40"}`}
                      >
                        {isCompleted ? (
                      ...
                      <div className="flex-1 min-w-0">
                        <h4
                          className={`font-black text-[11px] uppercase tracking-wide truncate ${isActive ? "text-white" : "text-white/70"}`}
                        >
                          {stage.title}
                        </h4>
                        <div className="flex items-center gap-1">
                          <div
                            className={`w-1.5 h-1.5 rounded-full ${isCompleted ? "bg-emerald-400" : isUnlocked ? "bg-yellow-400" : "bg-white/10"}`}
                          />
          ...
          <div className="hidden lg:flex p-6 bg-gradient-to-br from-orange-400/80 to-pink-500/80 backdrop-blur-md rounded-[2.5rem] shadow-2xl border border-white/20">
            ...
            <p className="text-white text-sm italic font-medium leading-relaxed">
              {gameState.isQuizActive
                ? "Hati-hati! Gerbang ini memerlukan ketelitian tinggi."
                : "Klik titik-titik di peta untuk mempelajari rahasia Matriks!"}
            </p>
          </div>
        </aside>
            ...
            {viewMode === "map" ? (
              ...
              >
                <div className="mb-4 md:mb-6 text-center">
                  <h2 className="text-2xl md:text-3xl font-black text-white drop-shadow-lg uppercase tracking-tight">
                    Peta Pulau Matriks
                  </h2>
                  <p className="text-white/60 font-medium tracking-widest text-[10px] uppercase">
                    Pilih lokasi tujuanmu
                  </p>
                </div>
                <MapView
                  stage={currentStage}
                ...
                <div className="flex-1 bg-white/20 backdrop-blur-2xl border border-white/30 rounded-[2.5rem] md:rounded-[3.5rem] p-6 md:p-12 shadow-2xl relative overflow-hidden flex flex-col">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 items-center flex-1">
                    {currentSubTopic.examples.map((ex, exIdx) => (
                      <div
                        key={exIdx}
                        className="bg-white/10 backdrop-blur-md rounded-[2.5rem] p-6 md:p-8 border border-white/20 shadow-xl flex flex-col items-center"
                      >
                        <h4 className="text-white/40 text-[10px] font-black uppercase tracking-[0.3em] mb-4 md:mb-8 text-center uppercase tracking-widest">
                          Kristal Matriks {exIdx + 1}
                        </h4>
                        <div className="scale-75 md:scale-90 lg:scale-100 origin-center">
                          <MatrixDisplay data={ex.data} />
                        </div>
                        <div className="mt-4 md:mt-8 px-6 py-2 bg-black/20 rounded-full border border-white/10">
                          <code className="text-pink-300 font-mono text-xs md:text-sm font-black">
                            {ex.label}
                          </code>
                        </div>
                      </div>
                  ...
                  <div className="mt-8 md:mt-12 flex justify-end items-center relative z-10">
                    <button
                      onClick={() => {
                        playSfx(
                          "https://assets.mixkit.co/active_storage/sfx/407/407-preview.mp3",
                        );
                        handleNextSubTopic();
                      }}
              ...
              >
                <div className="text-center">
                  ...
                  <span className="text-white/50 font-black text-[10px] tracking-[0.4em] uppercase">
                    Tantangan Penjaga Gerbang
                  </span>
                  <h3 className="text-4xl font-black text-white mt-2 leading-none">
                    TAHAP {currentQuizIndex + 1} / {shuffledQuizzes.length}
                  </h3>
                </div>
      ...
      <footer className="relative z-10 flex flex-col md:flex-row gap-4 mt-6">
        <div className="bg-white/10 backdrop-blur-xl rounded-2xl flex-1 border border-white/20 flex flex-wrap items-center px-6 md:px-8 py-4 gap-4 md:gap-8">
          <span className="text-white/40 font-black text-[10px] uppercase tracking-[0.2em] w-full md:w-auto">
            Saku Ilmu:
          </span>
          <div className="flex flex-wrap gap-3">
            <div className="px-4 py-2 bg-blue-500/40 rounded-xl border border-blue-400 text-white text-[10px] font-black shadow-lg">
              GANDA
            </div>
            <div className="px-4 py-2 bg-purple-500/40 rounded-xl border border-purple-400 text-white text-[10px] font-black shadow-lg">
              ESSAY
            </div>
            <div className="px-4 py-2 bg-pink-500/40 rounded-xl border border-pink-400 text-white text-[10px] font-black shadow-lg">
              TARIK
            </div>
            <div className="px-4 py-2 bg-orange-500/40 rounded-xl border border-orange-400 text-white text-[10px] font-black shadow-lg">
              ISIAN
            </div>
          </div>
        </div>
        <div className="bg-white/20 backdrop-blur-xl rounded-2xl px-10 py-4 border border-white/30 flex items-center justify-center shadow-xl mb-4 md:mb-0">
          <span className="text-white font-black text-xs md:text-sm tracking-widest uppercase text-center">
            PENGUASAAN: {masteryPercentage}%
          </span>
        </div>
      </footer>
